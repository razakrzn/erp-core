...
class EmployeeAdminForm(forms.ModelForm):
    create_user = forms.BooleanField(
        ...
        initial=False,
        help_text="Check this to create a login account for this employee.",
    )
    username = forms.CharField(required=False, help_text="Required if creating a user account.")
    password = forms.CharField(
        widget=forms.PasswordInput, required=False, help_text="Required if creating a user account."
    )
    role = forms.ModelChoiceField(
        queryset=Role.objects.all(),
        required=False,
        help_text="Select a role if creating a user account.",
        empty_label="---------",
    )
...
class EmployeeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Employee Details", {"fields": ("department", "designation", "is_active")}),
        (
            "Personal Info",
            {
                "fields": ("first_name", "last_name", "email"),
                "description": (
                    "Enter name and email here. If a user is created or linked, "
                    "these will be synced/moved to the User record."
                ),
            },
        ),
        (
